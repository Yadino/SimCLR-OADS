...
def run():
    ...
    for layer_name in all_activations:
        ...
        csv_str_header = "checkpoint, layer, accuracy"
        csv_str = f"{checkpoint_path}, {layer_name}, {accuracy*100}"
        ...
        class_labels = ['Balcony door', 'Bench', 'Bin', 'Bollard', 'Carrier bike', 'Compact car', 'Front door',
                        'Lamppost', 'Oma fiets', 'Scooter', 'SUV', 'Traffic light', 'Traffic sign', 'Tree', 'Truck',
                        'Van']
        # Replace numerical labels with class labels
        disp.ax_.set_xticklabels(class_labels, rotation=45, ha='right')
        disp.ax_.set_yticklabels(class_labels)

        plt.show()

        fig_path = os.path.join(layer_dir, f"confusion_matrix_{layer_name}.png")
        ...
        accuracy_filename = os.path.join(datetime_dir, "accuracy.txt")
